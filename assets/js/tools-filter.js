(() => {
  ...
  const initToolsDirectory = () => {
    ...
    const input = directory.querySelector("#tools-filter-input");
    const clearButton = directory.querySelector("[data-tools-filter-clear]");
    const status = directory.querySelector("[data-tools-filter-status]");
    const emptyState = directory.querySelector("[data-tools-empty-state]");
    const categoryButtons = Array.from(directory.querySelectorAll("[data-tools-category]"));
    const cards = Array.from(directory.querySelectorAll(".tool-card"));
    ...
    const updateResults = () => {
      ...
      if (!terms.length && !activeCategory) {
        ...
        const toolLabel = visibleCount === 1 ? "tool" : "tools";
        let message = `Showing ${visibleCount} ${toolLabel}`;
    ...
    directory.addEventListener("click", (event) => {
      const tag = event.target.closest("[data-tool-category-tag]");
      ...
      const nextCategory = tag.dataset.toolCategoryTag || "";
